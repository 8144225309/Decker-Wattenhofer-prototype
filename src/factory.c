...
/* Compute taproot-tweaked xonly pubkey (key-path only, no script tree). */
static int taproot_tweak_pubkey(
    ...
    if (!secp256k1_xonly_pubkey_from_pubkey(ctx, tweaked_out, NULL, &tweaked_full))
        return 0;
...
static int build_musig_p2tr_spk(
    ...
    if (!taproot_tweak_pubkey(ctx, tweaked_out, &keyagg_out->agg_pubkey))
        return 0;
...
static int build_single_p2tr_spk(
    ...
    if (!taproot_tweak_pubkey(ctx, tweaked_out, &internal))
        return 0;
...
int factory_build_tree(factory_t *f) {
    ...
    int kr  = add_node(f, NODE_KICKOFF, all, 5,        -1, 0, -1);
    int sr  = add_node(f, NODE_STATE,   all, 5,        kr, 0,  0);
    int kl  = add_node(f, NODE_KICKOFF, left_set, 3,   sr, 0, -1);
    int kri = add_node(f, NODE_KICKOFF, right_set, 3,  sr, 1, -1);
    int sl  = add_node(f, NODE_STATE,   left_set, 3,   kl, 0,  1);
    int sri = add_node(f, NODE_STATE,   right_set, 3, kri, 0,  1);
