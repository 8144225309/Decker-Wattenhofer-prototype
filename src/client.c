...
static int client_init_channel(channel_t *ch, secp256k1_context *ctx,
    ...
    /* Set up basepoints using same deterministic scheme as LSP expects.
       Client's "local" basepoints come from the client pubkey.
       Client's "remote" basepoints come from the LSP's derivation. */

    /* Client local basepoints (from client pubkey — same scheme LSP uses for "remote") */
    unsigned char client_pk_ser[33];
    size_t pk_len = 33;
    secp256k1_ec_pubkey_serialize(ctx, client_pk_ser, &pk_len, &my_pubkey,
                                   SECP256K1_EC_COMPRESSED);
    unsigned char pay_sec[32], delay_sec[32], revoc_sec[32], htlc_sec[32];
    {
        unsigned char buf[33 + 16];
        memcpy(buf, client_pk_ser, 33);
        memcpy(buf + 33, "payment\0", 8); sha256(buf, 41, pay_sec);
        memcpy(buf + 33, "delayed\0", 8); sha256(buf, 41, delay_sec);
        memcpy(buf + 33, "revocatn", 8); sha256(buf, 41, revoc_sec);
        memcpy(buf + 33, "htlcbase", 8); sha256(buf, 41, htlc_sec);
    }
    ...
    /* Shachain seeds removed: random per-commitment secrets generated by channel_init().
       Remote PCP will be set during basepoint exchange (before channel_init). */

    memset(my_seckey, 0, 32);
    memset(pay_sec, 0, 32);
    memset(delay_sec, 0, 32);
    memset(revoc_sec, 0, 32);
    memset(htlc_sec, 0, 32);
    return 1;
}
...
int client_do_factory_rotation(int fd, secp256k1_context *ctx,
    ...
    /* Send client's basepoints to LSP */
    {
        secp256k1_pubkey my_pub;
        secp256k1_keypair_pub(ctx, &my_pub, keypair);
        unsigned char cpk[33];
        size_t cpk_len = 33;
        secp256k1_ec_pubkey_serialize(ctx, cpk, &cpk_len, &my_pub, SECP256K1_EC_COMPRESSED);
        unsigned char ps[32], ds[32], rs[32], hs[32];
        {
            unsigned char buf[33 + 16];
            memcpy(buf, cpk, 33);
            memcpy(buf + 33, "payment\0", 8); sha256(buf, 41, ps);
            memcpy(buf + 33, "delayed\0", 8); sha256(buf, 41, ds);
            memcpy(buf + 33, "revocatn", 8); sha256(buf, 41, rs);
            memcpy(buf + 33, "htlcbase", 8); sha256(buf, 41, hs);
        }
        secp256k1_pubkey cpay, cdel, crev, chtlc;
        secp256k1_ec_pubkey_create(ctx, &cpay, ps);
        secp256k1_ec_pubkey_create(ctx, &cdel, ds);
        secp256k1_ec_pubkey_create(ctx, &crev, rs);
        secp256k1_ec_pubkey_create(ctx, &chtlc, hs);
        ...
        /* Generate random per-commitment secrets for cn=0 and cn=1 (outer-scoped rot_pcs0, rot_pcs1) */
        {
            ...
            if (urand_fd >= 0) {
                read(urand_fd, rot_pcs0, 32);
                read(urand_fd, rot_pcs1, 32);
                close(urand_fd);
            }
        }
...
int client_run_with_channels(secp256k1_context *ctx,
    ...
    if (channel_cb) {
        ...
        /* Send client's basepoints to LSP */
        {
            /* Generate client basepoints (same derivation as client_init_channel) */
            secp256k1_pubkey client_pay_bp, client_delay_bp, client_revoc_bp, client_htlc_bp;
            unsigned char client_pk_ser[33];
            size_t cpk_len = 33;
            secp256k1_ec_pubkey_serialize(ctx, client_pk_ser, &cpk_len, &my_pubkey,
                                           SECP256K1_EC_COMPRESSED);
            unsigned char ps[32], ds[32], rs[32], hs[32];
            {
                unsigned char buf[33 + 16];
            ...
            secp256k1_ec_pubkey_create(ctx, &client_pay_bp, ps);
            secp256k1_ec_pubkey_create(ctx, &client_delay_bp, ds);
            secp256k1_ec_pubkey_create(ctx, &client_revoc_bp, rs);
            secp256k1_ec_pubkey_create(ctx, &client_htlc_bp, hs);
            memset(ps, 0, 32); memset(ds, 0, 32);
            memset(rs, 0, 32); memset(hs, 0, 32);
               ...
               pcs_secret0, pcs_secret1 declared in outer scope. */
            {
                int urand_fd = open("/dev/urandom", O_RDONLY);
                if (urand_fd >= 0) {
                    read(urand_fd, pcs_secret0, 32);
                    read(urand_fd, pcs_secret1, 32);
                    close(urand_fd);
                }
            }
...
int client_run_reconnect(secp256k1_context *ctx,
    ...
    /* 7. Derive LSP's basepoint pubkeys for channel init.
       On reconnect the basepoints were already exchanged in the initial session.
       Since the LSP derives its basepoints deterministically from its pubkey,
       we can recompute them here. */
    secp256k1_pubkey reconn_lsp_pay_bp, reconn_lsp_delay_bp, reconn_lsp_revoc_bp, reconn_lsp_htlc_bp;
    {
        const secp256k1_pubkey *lsp_pk = &factory.pubkeys[0];
        unsigned char lsp_ser[33];
        size_t lsp_len = 33;
        secp256k1_ec_pubkey_serialize(ctx, lsp_ser, &lsp_len, lsp_pk,
                                       SECP256K1_EC_COMPRESSED);
        unsigned char ps[32], ds[32], rs[32], hs[32];
        unsigned char buf[33 + 16];
        memcpy(buf, lsp_ser, 33);
        memcpy(buf + 33, "payment\0", 8); sha256(buf, 41, ps);
        memcpy(buf + 33, "delayed\0", 8); sha256(buf, 41, ds);
        memcpy(buf + 33, "revocatn", 8); sha256(buf, 41, rs);
        memcpy(buf + 33, "htlcbase", 8); sha256(buf, 41, hs);
        secp256k1_ec_pubkey_create(ctx, &reconn_lsp_pay_bp, ps);
        secp256k1_ec_pubkey_create(ctx, &reconn_lsp_delay_bp, ds);
        secp256k1_ec_pubkey_create(ctx, &reconn_lsp_revoc_bp, rs);
        secp256k1_ec_pubkey_create(ctx, &reconn_lsp_htlc_bp, hs);
        memset(ps, 0, 32); memset(ds, 0, 32);
        memset(rs, 0, 32); memset(hs, 0, 32);
    }
